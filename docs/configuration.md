...
Routing is part of the plugin product surface. `capability_route` ranks installed plugin Skills, plugins, command adapters, and deferred MCP routes from bounded names, descriptions, aliases, Codex-style `agents/openai.yaml` metadata, default prompts, declared dependencies, exclusions, trust state, and implicit-invocation policy. After `open_workspace`, `tool_search(workspaceId=...)` adds project-local and user Agent Skills to the same direct/deferred competition and returns an exact `recommendedRoute.nextAction`. Routing changes emit MCP `notifications/tools/list_changed`; Stable Gateway also fingerprints descriptions, input/output schemas, UI/routing metadata, and model instructions so a stale session must reinitialize instead of retaining an obsolete route. See [Capability Routing Contract](capability-routing.md).
...
| `DEVSPACE_GOAL_ROUND_RECOVERY` | `1` | `goalRoundRecoveryEnabled` | Re-drive an interrupted Goal working round only in its exact bound conversation. Set to `0` for a temporary manual-reentry hold without pausing or rebinding the Goal. |
...
All four features support **Chat mode only**. Stream Recovery never reloads merely because a model is slow: normal renderer progress cancels recovery, the same conversation must remain active, server status must be `COMPLETE`, and recovery is one-shot/cooldown protected. Context Guardian uses the active Classic-native model window plus the strongest explicitly labelled host/snapshot/ledger pressure signal rather than a fixed global ceiling. At rollover pressure it creates a bounded selective capsule and arms either the next real user request or an already-authorized hidden Goal continuation. ChatGPT may assign a new backend conversation ID, but UI continuity is accepted only after source/carry/target reduction, hidden capsule markers, Goal/Plan frontier, native authority, progress narration, and Host Overlay transfer all verify. Exact token fields remain unavailable rather than falling back silently. Host Overlay is a projection only: GoalRuntime and PlanRuntime remain the sole state authorities. Every observed Main conversation receives exactly one conversation-scoped floating progress narration card, including an idle placeholder before its first tracked work event; Goal, Plan, transcript, and tool-progress rows are never borrowed from another conversation. The floating Goal strip, Plan HUD, and narration card are the canonical user-facing surfaces. The older transcript Goal Dock and Plan Card resources remain registered only for cached-schema compatibility, but Goal/Plan tools no longer render them inline, and existing legacy iframe or failed-template placeholders are suppressed without reloading or navigating the conversation.
