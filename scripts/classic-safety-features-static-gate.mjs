...
assert.match(server, /if\s*\(!config\.passiveCore\)[\s\S]*if\s*\(config\.goalRoundRecoveryEnabled\)[\s\S]*goalRoundCompletionGuard\.start/, "passive Core or an explicit Goal recovery hold must not start Goal Round Completion Guard");
