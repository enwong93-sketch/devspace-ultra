...
export function evaluateParity({ codexText, devspaceText }) {
  const rows = [
    ...
    },
    {
      ...
      priority: "P2",
      codexAny: ["danger-full-access", "sandbox"],
      devspaceAll: ["executionPolicy", "full-access", "exec_command"],
      target: "complete",
      note: "DevSpace intentionally exposes one local full-access execution policy, with no sandbox permission round-trip. Tool allow/deny filters and host safety rules remain separate.",
    },
    {
      id: "durable-memory",
      priority: "P2",
