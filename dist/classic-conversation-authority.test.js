...
try {
  ...
  const legacyDirectPath = join(root, "legacy-direct-authority.json");
  await writeFile(legacyDirectPath, JSON.stringify({
    version: 1,
    sessions: [{
      fingerprint: directFingerprint,
      conversationIds: ["dddddddd-dddd-dddd-dddd-dddddddddddd"],
      runtimeKeys: ["Main-02"],
      ambiguous: false,
      updatedAt: "2026-09-06T03:53:00.000Z",
      verifiedDirectSession: true,
      verifiedDirectSessionAt: "2026-09-06T03:53:00.000Z",
    }],
  }), "utf8");
  ...
  assert.equal(currentRoute?.verifiedDirectSession, false);
  assert.equal(registry.diagnostics().verifiedDirectSessions, 0,
    "native route telemetry must not create reusable direct-session authority");
...
console.log(JSON.stringify({ ok: true, gate: "classic-conversation-authority", nativeOnly: true, durableDirectSessionAuthorityRetired: true, ambiguityFailsClosed: true }));
