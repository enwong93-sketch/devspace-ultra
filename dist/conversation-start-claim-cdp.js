...
class ClaimCdpClient extends ClassicCdpClient {
  ...
  async open() {
    ...
    this.on("Runtime.executionContextsCleared", () => { this.contexts = []; });
  }
}
...
async function fetchTargets(port, { fetchImpl = globalThis.fetch, timeoutMs = DEFAULT_PROBE_TIMEOUT_MS } = {}) {
  if (typeof fetchImpl !== "function") return [];
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  timer.unref?.();
  try {
    const response = await fetchImpl(`http://127.0.0.1:${port}/json/list`, { cache: "no-store", signal: controller.signal });
...
function claimProbeExpression(claimId) {
  const expected = JSON.stringify(claimId);
  return `(() => {
    const expected=${expected};
    const values=[globalThis.openai?.toolOutput,globalThis.openai?.toolResponseMetadata,globalThis.openai?.toolResult];
    const read=(value)=>value?.structuredContent?.progressClaim?.claimId
...
async function evaluateClaimTarget(target, claimId, {
  ...
  try {
    ...
    const result = await client.call("Runtime.evaluate", {
      ...
      expression: claimProbeExpression(claimId),
      returnByValue: true,
      awaitPromise: false,
    });
...
export class ProgressClaimCdpResolver {
  constructor({
    ports = defaultMainDebugPorts(),
    listTargets = (port) => fetchTargets(port),
    evaluateTarget = (target, claimId) => evaluateClaimTarget(target, claimId),
    maxIframes = DEFAULT_MAX_IFRAMES,
    batchSize = DEFAULT_BATCH_SIZE,
    now = () => Date.now(),
  } = {}) {
    this.ports = [...new Set((Array.isArray(ports) ? ports : []).map(Number).filter(Number.isInteger))];
    ...
    for (const port of this.ports) {
      ...
      for (let index = 0; index < iframes.length; index += this.batchSize) {
        ...
        const results = await Promise.all(batch.map(async (target) => ({
          target,
          matched: await this.evaluateTarget(target, expected).catch(() => false),
        })));
...
export const progressClaimCdpInternals = { claimProbeExpression, cleanClaimId, conversationIdFromUrl, chooseAppContext };
