...
export function createServer(config = loadConfig(), options = {}) {
    ...
    const codexMcpBridge = new CodexMcpBridge({ codexHome: config.agentDir });
    const codexSandboxRuntime = new CodexSandboxRuntime({
        stateDir: config.stateDir,
    ...
    return {
        ...
        close: () => {
            closePromise ??= (async () => {
                ...
                await codexSandboxRuntime.close();
