...
function createMcpServer(config, workspaces, reviewCheckpoints, processSessions, localAgentProviders, incomingArtifactAdapters, chatSwarm, browserControl, capabilityRuntime, blenderRuntimeManager, codexMcpBridge, conversationContinuity, contextGuardian, exactUsageAuthority, codexContextBridge, planRuntime, goalRuntime, goalHostBridge, hostOverlayProjection, conversationAuthority, conversationAuthorityReady, goalRunProgress, requestConversationContext, conversationProgressLiveness = null) {
    ...
    server.registerTool("devspace_progress_report", {
        ...
        description: "Write one concise, conversation-bound update to the floating DEV Space progress narration card in your own natural language. Write after each meaningful medium-sized step, important verification, material direction change, or genuine blocker, and during ongoing non-atomic work keep the interval between your own reports below ten minutes. Ten minutes is an Agent reporting ceiling only; no timer or supervisor sends a reminder or creates a synthetic user turn. Do not report every tool call or mechanically emit boilerplate on a fixed count. Keep the wording completely free-form and specific to what just became true. Do not mirror low-level tools, counters, heartbeats, templates, or program status. The tool waits for the current ChatGPT Classic conversation identity instead of failing on a short correlation deadline; it never accepts another conversation id and cannot write across chats.",
        inputSchema: {
