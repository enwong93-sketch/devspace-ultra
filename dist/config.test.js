...
try {
  ...
  assert.equal(loadConfig(baseEnv).goalRoundRecoveryEnabled, false,
    "visible same-round Goal Recovery is retired because Goal control metadata must never reach the composer");
  assert.equal(loadConfig({ ...baseEnv, DEVSPACE_GOAL_ROUND_RECOVERY: "true" }).goalRoundRecoveryEnabled, false,
    "legacy env/config values must not revive the retired visible Goal recovery transport");
