...
- Fixed the secondary-Main progress fallback that could leave `DEVSPACE 進度旁白` at `0 段`. The hidden claim App was mounting, but affected ChatGPT Desktop builds exposed `window.openai.callTool` while leaving `toolOutput` empty. `devspace_progress_report` now declares its structured output shape and mirrors only the opaque one-time claim descriptor into app-only tool-result metadata; the relay accepts either host binding while the exact-page and cross-conversation fail-closed checks remain unchanged.
- Added a transactional product self-updater for existing DevSpace Ultra installations. `devspace update` resolves the latest stable GitHub Release, verifies the release archive SHA-256, stages and validates it outside the live global package, migrates legacy `@waishnav/devspace` installs to the canonical `devspace-ultra` package root, backs up the previous package and npm shims, and rolls back a failed post-swap verification.
- Added same-version maintenance refresh support keyed by the release archive digest rather than semantic version alone, so corrected `0.5.8` payloads can replace earlier `0.5.8` payloads safely. Successful Windows installs register a daily randomized `DevSpace-Ultra-Auto-Update` task; automatic runs defer while Stable Gateway reports active non-stream Agent/tool work.
- Added a digest-verified `update.ps1` GitHub Release asset for builds that predate the updater. Current `install.ps1` detects an existing official installation and routes it through the transactional updater instead of blindly reinstalling over the live package.
- Added sandbox migration/rollback gates with explicit `DEVSPACE_UPDATE_TEST_MODE` isolation. Test runs cannot inspect, stop, start or register production DevSpace Scheduled Tasks, cannot rewrite the installed Agent Skill, and prove production Gateway/Core/Main listener ownership remains unchanged across legacy migration and an injected rollback failure.
