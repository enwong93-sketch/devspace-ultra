...
- Promoted interactive progress narration from a prompt-only convention to a Local Gateway product gate for exact user-facing Main conversations. An active Plan now requires a verified current-turn `devspace_progress_report` before substantive tools; an unplanned turn gets only one atomic substantive-tool exception before narration becomes mandatory; reports older than the ten-minute ceiling block the next substantive phase; and Plan completion requires a fresh verified report. The exact-page compatibility bridge satisfies the same gate through durable ownership proof, while backend Chat Swarm workers remain excluded and no timer/supervisor is allowed to synthesize user-facing prose.
- Fixed the first direct Codex Computer Use call after a fresh tool/session bind. When native `call_mcp` correlation has not arrived yet, only `codex_computer_use_status` and `codex_computer_use` may use a request-scoped fallback, and only when exactly one hydrated Main page is visibly generating, has an empty composer, and owns a progress card for that same conversation. Two active pages, a mismatched card owner, or an idle page fail closed; the fallback never creates durable session authority.
- Added a conversation-scoped blue Computer Use takeover state to ChatGPT Classic. While the official bundled `@oai/sky` runtime is observing or operating a permitted Windows app, the current conversation shows the target app, action and `OpenAI @oai/sky`; the layer never intercepts user input, consecutive actions share one bounded session, one Core owns the UI, and the state clears after ten idle seconds or a hard timeout. ChatGPT, Codex, terminals and authentication/security surfaces remain prohibited.
- Added an explicit Computer Use release handshake. The Agent must finish with a read-only re-observation carrying `input.release_control=true`, which clears the blue takeover state immediately before the visible final response or before switching to unrelated work. Mutating actions cannot request release because they still require a post-action observation, and any failed Computer Use operation fail-safe releases the takeover state instead of leaving the user-facing ownership indicator behind.
- Fixed the final long-turn authority/rescue edge cases: canonical page-local tool invocations remain valid across a transient idle tool boundary instead of being rejected solely because the visible generating affordance dropped; HTTP `loadingFinished` is explicitly non-terminal for tool-using assistant turns; and a genuinely interrupted turn stuck behind a stale Stop/generating UI may reset that stale affordance once after the twenty-minute rescue gate before the existing one-shot exact-conversation `- 繼續` recovery runs. A cancellation generated by that guarded reset no longer masquerades as a new user cancellation, while ordinary user cancellation still disarms rescue immediately.
- Strengthened the Agent-authored progress contract around long waits. Before entering an external process/CI watch that could carry the latest narration past the ten-minute ceiling, the working Agent must report first, and after the wait returns it must report the material result before another long phase; timers and supervisors still never author narration prose.
- Removed a recurring arm64 macOS CI race in the Stable Gateway OAuth-token regression. The assertion now selects the exact translated request id instead of assuming that request remains the last diagnostic row after an asynchronous schema-fingerprint probe.
...
- Kept Agent-authored progress history across later assistant turns and Stable Gateway/Core restarts. The durable conversation state was intact, but the overlay incorrectly hid messages older than thirty minutes and could remount as `0 段`; conversation ownership and bounded message-count retention now control projection instead of wall-clock age.
- Treat an armed exact-conversation episode that survives Core replacement as restart interruption evidence. Normal completion or a fresh native turn still disarms or replaces it, but after the full twenty-minute gate a stale generating affordance can be reset once and the existing one-shot `- 繼續` rescue can complete instead of remaining permanently blocked.
- Fixed the remaining twenty-minute rescue deadlock when ChatGPT stays visually `generating` forever without emitting a transport failure or visible page error. An exact hydrated page whose latest visible turn is still the user's request now becomes `stalled-generating` evidence only after twenty minutes with no Agent-authored report and no newly observed substantive tool call; a second exact-page confirmation is required before resetting the stale Stop affordance, then the existing one-shot `- 繼續` path runs. Fresh tool activity resets only that conversation's clock, concurrent Mains remain isolated, and Core replacement preserves already elapsed silence instead of restarting the twenty-minute timer.
