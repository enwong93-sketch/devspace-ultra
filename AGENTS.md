...
Goal round closure is also a backend product gate, not merely an instruction for the model to remember. `devspace_goal_turn_report` is rejected while the current turn Plan remains active. Once the current-round Plan is completed while the Goal is still `active / working`, unrelated substantive tools are rejected until the Agent either calls `devspace_goal_turn_report` as the final tool and then emits one visible final report, or explicitly starts a fresh turn Plan because meaningful work genuinely remains. A completed Plan from an earlier round never closes a newer round. If the Agent nevertheless ends a visible turn before either action, hidden same-round Goal Recovery must resume the exact conversation; no visible Goal-control prompt or synthetic user turn is permitted.
